class Canal{
 ...
 CheckBox curve; // suavizar as curves com curveVertex() / vertex()
 CheckBox messe; // messe tempo e tensão
 ...
 float xi,yi,dx,dy; // retangulo de messe tempo e tensão na tela
 ...
 Canal(byte n_,color nCor_, int x_, int y_, int w_, int h_){
     ...
     p0Trigger=p0;
     messe=new CheckBox("messe",ft.x,ft.y+ft.h+5,15);
     curve=new CheckBox("curve",ft.x+ft.w/2,ft.y+ft.h+5,15);
 }
  ...
  void display(){
    ...
    //println("n=",n);
     ...
     if (chN.clicado && (!chXYZ.XYZ.clicado || chXYZ.verCanais.clicado)){
      ...
      //if (calcFreq.clicado) analisarcurve(); else {qPicos=0; qVales=0;}
        //tirei o analisarcurve() em 19/09/15 para por o curveDiferencial
...
*/    
 ...
 /*  tCalc.setV(0);
   ...
   if (pMax1>=0 && pMax2>=0){
    ...
    text(fCalc.printV()+"Hz ("+tCalc.printV()+"s)",messe.x,messe.y+29); 
   }
  */ 
    ...
    void displayRect(){ 
     ...
     void telaMousePressionou(){
      ...
      // println("cor=",get(mouseX,mouseY));
       if (messe.clicado){ // acertar procurar qual cor de canal mais próximo ao mouse
  ...
  boolean mouseClicado(){
     ...
     fm.mouseClicado();
     ft.mouseClicado();
     if (messe.mouseClicado()){
        if (messe.clicado){
           for (int k=0;k<4;k++){
              canal[k].messe.clicado=false; 
           }
           messe.clicado=true;
        }
     };
