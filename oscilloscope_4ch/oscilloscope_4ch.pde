...
CheckBox verPontos; // ver os pontos da Sample
...
// painel para messe resistor/capacitor
...
void setup() {
  ...
  resetaxis=new Botao("axis",marg1+50,tela.y,45,20);
  resetmesse=new Botao("messe",resetaxis.x+resetaxis.w+2,tela.y,45,20);

  //demo & canais
  for (byte k=0; k<4; k++) {
    if (k<3) {demo[k]=new Botao(str(k+1), marg1+50+k*30, tela.y/2-10, 30, 20);}
    //messe[k]=new Botao(str(k),marg2+60+k*(18+2),tela.y+10,18,20,cor[k],cor[k]);
    //trigger[k]=new Botao(str(k),messe[k].x,messe[k].y+messe[k].h+10,messe[k].w,20,cor[k],cor[k]);
    canal[k]=new Canal(k, cor[k], marg1, tela.y+27+k*110, 143, 100); //h115
  }
  //messe[4]=new Botao("x",messe[3].x+messe[3].w,messe[3].y,messe[3].w,messe[3].h,color(200),color(0));
  ...
  verPontos=new CheckBox("show points", chXYZ.x, chXYZ.y+chXYZ.h+5, 15);
  calcFreq=new CheckBox("detect freq.", verPontos.x, verPontos.y+verPontos.h+5, 15);
  grafDif=new CheckBox("show",calcFreq.x+calcFreq.w,calcFreq.y,15);
  ...
  RC=new CheckBox("messe res./cap.", pnlRC.x, pnlRC.y, 15);
  ...
  sinal=new CheckBox("Gen.Sinal", pnlSinal.x, pnlSinal.y, 15);
  ...
  // posicionamento da Sample 
  pnlAmostra=new Painel("Sample", pnlSinal.x+pnlSinal.w+10, pnlSinal.y, 200, 85);
  ...
  oneAmostra=new Botao("one", dt.x, dt.y+dt.h+5, 50, 20);
  variasAmostras=new Botao("varied", oneAmostra.x+oneAmostra.w+5, oneAmostra.y, oneAmostra.w, oneAmostra.h);
  fluxoContinuo=new Botao("flow", variasAmostras.x+variasAmostras.w+5, variasAmostras.y, variasAmostras.w, variasAmostras.h);
...
void draw() {
  ...
  text("RESET",resetaxis.x-5,resetaxis.y+resetaxis.h/2);
  //text("messe", messe[0].x-5,messe[0].y+messe[0].h/2);
...
void mouseClicked() {
  ...
  if (resetaxis.mouseClicado()){
    for (int k=0; k<4;k++){
     canal[k].p0=tela.y+3*Q*(k+1);//posição da tensão zero
    }
    resetaxis.clicado=false;
  }
  
  if (resetmesse.mouseClicado()){
     for (int k=0; k<4;k++){
        canal[k].telaClicou=false; 
     }
     resetmesse.clicado=false;
  }
  ...
  */
  ...
  // botões para messe tempo x tensão nos 4 canais (e botão de limpar x)
  /*
  for (int k=0;k<5;k++){
    if (messe[k].mouseClicado()){
       if (messe[k].clicado){
          for (int j=0; j<k;j++){
             messe[j].clicado=false; 
          }
          for (int j=3;j>k;j--){
            messe[j].clicado=false;
          }
  ...
  */
  ...
  if (oneAmostra.mouseClicado()) { // receber apenas one Amostra
    variasAmostras.clicado=false;
    fluxoContinuo.clicado=false;
    if (com.conectado) {
      port.write("1"); 
    }
    oneAmostra.clicado=false;
...
void mousePressed() {
  ...
  resetaxis.mousePressionou();
  resetmesse.mousePressionou();
...
void mouseReleased() {
  ...
  resetaxis.mouseSoltou();
  resetmesse.mouseSoltou();
  // só para aparecer o verde do pressionado
  oneAmostra.mouseSoltou();
  ...
  if (tex.charAt(0)=='>') { //comando: >cmd=v1(tab)v2(tab)v3(tab)
    ...
    if (i>=0) { // encontrou sinal "=" (igual)  obs: i=-1 => não encontrou o sinal '='
      ...
      if (cmd.equals("f")) { // entra fluxo de dados - deslocar dados e armazenar no final
        ...
        } else if (cmd.equals("one")){ // val= 0 ou 1
          //oneAmostra.clicado=boolean(int(val));
