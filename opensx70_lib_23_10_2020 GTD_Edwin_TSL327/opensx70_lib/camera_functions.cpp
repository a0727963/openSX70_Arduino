...
void Camera::mirrorUP()
{
  ...
  {
    #if BASICDEBUG
     Serial.println("Wait for S5 to get open");
    #endif
...
{
  pinMode(pin, INPUT_PULLUP); //added on 23.10.2020 because of an issue with reading out S5
...
void Camera::AutoExposure(int _myISO)
{
    ...
    //startCounter();
    meter_init();
    meter_integrate(); //Do the Integration
    Camera::shutterOPEN ();
    #if EXPDEBUG
