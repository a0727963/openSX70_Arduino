#include "Arduino.h"
#include "open_SX70.h"
//Version 12_10_2020_Edwin_TCS3200 and UDONGLE 6.6SP
...
void setup() {//setup - Inizialize
  #if DEBUG
    Serial.begin (9600);
    Serial.println("Welcome to openSX70 Version: 12_10_2020_Edwin_TCS3200 GTD-V2 and UDONGLE");
...
void loop() {//loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop
  ...
  //Start Picture Taking Functions
  noDongle();
  flashOperation();
  positionB();
  positionT();
...
void switch2Function(int mode){ //Selftimer
...
void checkFilmCount(){
  if ((currentPicture == 8) || currentPicture == 9){
      #if SIMPLEDEBUG
        Serial.print("Two Frames left!, currentPicture on Two Frames left: ");
