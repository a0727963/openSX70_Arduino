#include "open_sx70.h"
  ...
  const uint8_t PIN_OE = 9;         //Pin 13 on edwin2  (PB1-OC1A)
  const uint8_t TCS3200_S1_Pin = 2; //Pin 32 on edwin2  (PD2-INT0)
  const uint8_t TCS3200_S3_Pin = 6; //Pin 10 on edwin2 (PD6-AIN0)
                                    //Pin 9 is LM on edwin2
  ...
  nearest(int x, int myArray[], int elements, bool sorted) //estimate the correct Slot for the Estimated Exposure Value
  {
  ...
  predictSlot(int predExpVal, int shutterSpeeds[], int slots, bool sorted) //int slot = nearest(PredictedExposure, ShutterSpeed, 11, false); //Calculate the slot
