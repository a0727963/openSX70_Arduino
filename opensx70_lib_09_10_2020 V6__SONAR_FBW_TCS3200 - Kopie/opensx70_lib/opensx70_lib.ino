#include "Arduino.h"
#include "open_SX70.h"
//Version 09_10_2020_SONARF_BW_TCS3200 V5 and UDONGLE
...
#endif
#if SONAR
//bool GTD = 0; //workaround -- temporarily 1 should be 0
bool FT = 0;
bool S1F = 0;
...
#if DEBUG
  Serial.begin (9600);
  Serial.println("Welcome to openSX70 Version: 09_10_2020_SONAR_FBW_TCS3200 V4 and UDONGLE");
#endif
  myDongle.initDS2408();
...
void preFocus() {
  if ((digitalRead(PIN_S1F) == HIGH) && (isFocused == 0)) { // S1F pressed
    ...
    //Serial.print("preFocusGTD: ");
    //Serial.println(openSX70.getGTD());

    while(openSX70.getGTD()!=1){
      //Serial.println(analogRead(PIN_GTD));
...
void unfocusing(){
  ...
  if ((currentPicture != currentPicOnFocus)  && (isFocused == 1)){     //currentPicture should be after picture taking +1
    ...
    //Serial.print("GTD: ");
    //Serial.println(digitalRead(PIN_GTD));
    isFocused = 0;
    turnLedsOff(); //WHY?
  }
}
