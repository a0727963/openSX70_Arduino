...
void setup() {//setup - Inizialize
#if DEBUG
  Serial.begin (9600);
  Serial.println("Welcome to openSX70 Version: 19_10_2020_SONAR_FBW_TCS3200 GTD and UDONGLE - Focus on Selftimer End");
  ...
  inizialized++;
  Serial.print("S8: ");
  Serial.print(digitalRead(PIN_S8));
  Serial.print("S9: ");
  Serial.print(digitalRead(PIN_S9));
...
void loop() {//loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop
  ...
  prevDongle = nowDongle;
    ...
    //Serial.print(digitalRead(PIN_S8));
    //Serial.print("S9: ");
    //Serial.println(digitalRead(PIN_S9));
    ispackEmpty();
    DongleInserted();
