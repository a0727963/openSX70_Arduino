...
#if DEBUG
  Serial.begin (9600);
  Serial.println("Welcome to openSX70 Version: 21_10_2020_SONAR_FBW-2_TCS3200 GTD and UDONGLE");
...
void loop() {//loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop
  ...
  //printReadings();
  normalOperation(); //Update swS1.update() only if S8 and S9 are open
...
void printReadings() {
  ...
  Serial.print("S1T");
  Serial.print(digitalRead(PIN_S1));
  Serial.print(" | Is Focused: ");
  Serial.print(isFocused);
  Serial.print(" | GTD: ");
  Serial.print(digitalRead(PIN_GTD));
  Serial.print(" | S1F: ");
