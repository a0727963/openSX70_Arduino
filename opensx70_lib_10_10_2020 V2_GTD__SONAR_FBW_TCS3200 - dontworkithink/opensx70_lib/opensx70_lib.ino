#include "Arduino.h"
#include "open_SX70.h"
//Version 10_10_2020_SONARF_BW_TCS3200 V2 and UDONGLE
...
#endif
#if SONAR
//bool GTD = 1; //workaround -- temporarily 1 should be 0
bool FT = 0;
bool S1F = 0;
...
#if DEBUG
  Serial.begin (9600);
  Serial.println("Welcome to openSX70 Version: 10_10_2020_SONAR_FBW_TCS3200 V2 and UDONGLE");
#endif
  myDongle.initDS2408();
...
void preFocus() {
  if ((digitalRead(PIN_S1F) == HIGH) && (isFocused == 0)) { // S1F pressed
    openSX70.S1F_Focus(); //Not real nececarry to write it the whole time?!
...
#if SIMPLEDEBUG
    Serial.print("Two Frames left! currentPicture: ");
...
#if SIMPLEDEBUG
    Serial.print("Ten Frames shot! currentPicture: ");
    Serial.println(currentPicture);
#endif
...
#if SIMPLEDEBUG
      Serial.print("FILMPACK IS EMPTY - S9 Closed, Current Picture on Empty Pack taken: ");
      Serial.println(currentPicture);
#endif
    }
  }
...
void normalOperation() {
  ...
  if (digitalRead(PIN_S8) == LOW && digitalRead(PIN_S9) == LOW) //S8 and S9 are OPEN
  {
    ...
    // open  closed --> FILMCOUNTER REACH 0 (NO FLASH)
...
void positionB() {
  ...
  if (((digitalRead(PIN_S1) == S1Logic) && ((ShutterSpeed[selector] == (POSB))) && (openSX70.getGTD() == 1))) //////////////POSITION B
  {
    #if SIMPLEDEBUG
...
void positionT() {
  ...
  if ((digitalRead(PIN_S1) == S1Logic) && (((ShutterSpeed[selector] == (POST)) && (openSX70.getGTD() == 1)))) //////////////POSITION T
  {
    #if SIMPLEDEBUG
