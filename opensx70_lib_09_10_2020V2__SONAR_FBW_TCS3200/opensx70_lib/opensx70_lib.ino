#include "Arduino.h"
#include "open_SX70.h"
//Version 09_10_2020_SONARF_BW_TCS3200 V2 and UDONGLE
...
#if DEBUG
  Serial.begin (9600);
  Serial.println("Version: 09_10_2020_SONAR_FBW_TCS3200 V2 and UDONGLE");
#endif
  myDongle.initDS2408();
...
void loop() {//loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop loop
  ...
  /*getS1F();
    ...
    if (!S1F) //S1F == 0 S1F not Pushed
    {
    if (isFocused)
    {
      Unfocus();
    }
    return; //Stop loop here if S1F is not pressed
    }

    //from here on only if S1F == 1
    if (!isFocused) //not focused
    {
    DoFocus();
    }
  */
...
void preFocus() {
  ...
  if ((digitalRead(PIN_S1F) == LOW) && (isFocused == 1)) { // S1F pressed
