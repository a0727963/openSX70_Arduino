#include "open_sx70.h"
  ...
  void tcs3200_init(){
    ...
    //S2 (Photodiode type selection pin) & S0 (Output frequency scaling selection pin) should be high, can be modified via jumper in PCB 
    ...
    else
    {
      ...
      if (myMillis  >= _interval )
      {
        ...
        if(PredExp>40000){ //bigger then a reliable Value | doesnt know if its needed
            #if LMDEBUG
            Serial.println("Exception: PredExp > 40000 ");
            #endif
            return -1;
          }
        return PredExp;
  ...
  int meter_compute(unsigned int _interval,int _activeISO) //Light Meter Helper Compute uses assigned ISO parameter
    {
      ...
      return -1; //return -1 for first run(dont computed, only initialize)
  }
